...
- `/profile` — Profile page: sub-heading tagline → "About Me" section (lead paragraph on ViMET timeline + collab invitation, then a `<figure>` with the Cardinal-walk photograph from `attached_assets/3c6ba7fc-…JPG` imported via `@assets` alias — float-right on md+, full-width stacked on mobile, with italic caption "The Cardinal walk — June 19, 2025…" — followed by 4-paragraph long-form origin essay: (1) Cardinal walk on June 19, 2025, (2) the Perception/Literal Wall framing + December 2025 Visual Metonymy discovery + early-2026 reading list (Qwen2.5-VL, NodeRAG, GRPO, NF4 QLoRA, Grounding DINO, continuous-token-reasoning) + bet on latent reasoning for on-device VLMs, (3) "perpetual beta" + 16GB VRAM design spec + 2k-image Kaggle distillation probe + open call for training-pipeline collaborators, (4) Hare Krishna/Paramatma context closing with Apple as next chapter) → Living Profile widget (fetches `VITE_LIVING_PROFILE_URL` on mount; manual Refresh button; falls back to hardcoded `https://living.rkrithika.me/living-profile` if env var unset; shows loading state, error state, summary as markdown, and a collapsible `<details>` "Full profile" section)
